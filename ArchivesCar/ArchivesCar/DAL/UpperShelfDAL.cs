...
{
     public class UpperShelfDAL
    {
        public bool UpperShelf(ref object errorMsg)
        {
            try
            {
                ReturnInfo returnInfo = new ReturnInfo() { TrueOrFalse = false };
                string url = string.Format(@"http://{0}:{1}/storeroom/countingcar/binding",PublicData.ServerConfig.serverIP,PublicData.ServerConfig.serverPort);
